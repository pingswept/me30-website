...
**dir(board)**: will report out the names of the all the pins available on your particular CircuitPython board

**xyz = digitalio.DigitalInOut(board.[pin name or number])**: calls up the digitalio library and creates a new variable xyz that will be equal to the voltage at a specific pin on a microcontroller board.  The pin names and numbers are on the board’s pinout diagram, e.g, D6, D5, LED, etc.

**xyz.direction = digitalio.Direction.OUTPUT**: tells the microcontroller that the digital variable xyz is a voltage output, not an input

**xyz.direction = digitalio.Direction.INPUT**: tells the microcontroller that the digital variable xyz is a voltage input

**xyz.value**: reports the voltage value at the pin corresponding to digital variable xyz

**xyz.value = 1**
    
**xyz.value = True**: both of these commands set the voltage for the digital variable xyz to be “high,” which in the case of the Feather microcontroller means 3.3 V

**xyz.value = 0**
    
**xyz.value = False**: both of these commands set the voltage for the digital variable xyz to be “low,” i.e., ground or 0 V
...
creates a new pulse-width modulation object called “xyz” that will hold all the information about sending out pulse-width modulation at a particular pin 
